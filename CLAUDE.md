...
### 2026-06-08 11:28 - 修复照片轮播不显示的根本原因
**问题**: 网页底部图片轮播窗口无法正常显示图片(7/24张加载失败)
**根因**: 7张照片扩展名为 `.png` 但实际文件格式为 JPEG。GitHub Pages 根据扩展名设置 `Content-Type: image/png`，浏览器解码 PNG 时遇到 JPEG 字节流 → 渲染失败。轮播每5秒切换一次，7张破损图片穿插其中，用户频繁看到空白。
**修复**: 重命名 7 个 `.png` → `.jpg`，重新生成 `photos.json` 并推送。GitHub Pages 正确返回 `image/jpeg`，浏览器正常解码。
**经验**: `generate_photos_json.py` 支持所有 `.{jpg,jpeg,png,gif,webp,heic}` 扩展名但不校验文件内容格式。建议在 `photos.json` 生成脚本中增加格式校验。
**测试**: 新增 `run_tests.py` T7 测试套(格式匹配 + Git跟踪 + 路径完整性)，73/73 ✅。
